...
class Ui_GroupBox(object):
    def setupUi(self, GroupBox):
        ...
        #QtCore.QObject.connect(self.lineEdit, sig1, self.textEdit.setText)
        #QtCore.QObject.connect(self.pushButton, QtCore.SIGNAL("clicked()"), self.textEdit.clear)
        #QtCore.QObject.connect(self.pushButton, QtCore.SIGNAL("clicked()"), self.lineEdit.clear)
    ...
    def searchZomato(self):
        ...
        print ('\n\n')
        try:
           self.details=self.p.search(q=self.restName,city=self.cityName,count=10)
    ...
    def bkp_processOutput(self,output):
        output=self.output
        #print ('THE DICTIONARY OUTPUT:-',self.output)
        noOfRestFnd=len(self.output)
        formatOutput='*' * 100
        print (formatOutput)
        print (self.output.keys())
        print (self.output.values())
        print (formatOutput)
        with open('zomato_rest_output.txt') as f:
             for k,v in self.output.items():
                 ...
                 print ('Average Price for 2:- ',d['avgPrice2']) 
                 '''
                     f.write('Name:- '+d['Name']+'\n')
                     f.write('Address:- '+d['address']+'\n')
                     f.write('Rating:- '+d['rating']+'\n')
                     f.write('Average Price for 2:- '+d['avgPrice2']+'\n')
